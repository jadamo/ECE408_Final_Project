...
void unroll_x(int C, int H, int W, int K, int b, float* x, float* x_unroll){
    // H, C, W -> H, (C x W)
    int H_out = H - K + 1;
    int W_out = W - K + 1;

    dim3 gridDim(C * H_out * W_out, 1, 1);
    dim3 blockDim(C * H_out * W_out / CUDA_MAX_MUM_THREADS, 1, 1);
    unroll_x_kernel<<<gridDim, blockDim>>>(C, H, W, K, b, x, x_unroll);
}

void unroll_w(int C, int K, int M, float* w, float* w_unroll){

  // M, C, K, K -> M, (C x K x K)
  dim3 gridDim(ceil(M*1.0 / TILE_WIDTH) ,ceil(C*1.0 / TILE_WIDTH), 1);
  dim3 blockDim(TILE_WIDTH, TILE_WIDTH, 1);
  unroll_w_kernel<<<gridDim, blockDim>>>(C, H, W, K, w, w_unroll);
}

__global__ void unroll_w_kernel(int C, int K, int M, float* w, float* w_unroll){
...
__global__ void unroll_x_kernel(int C, int H, int W, int K, int b, float* x, float* x_unroll){
                         ...
                         const mshadow::Tensor<gpu, 4, float> &w)
{
    ...
    float* x_unrolled, w_unrolled;
    cudaMalloc((float**) x_unrolled, W_unroll * H_unroll * sizeof(float));
    cudaMalloc((float**) w_unrolled, M * C*K*K * sizeof(float));

    unroll_w(C, K, M, w.dtpr_, d_unrolled);

    for (int b = 0; b < B; b++){
        unroll_x(C, H, W, K, b, x.dtpr_, x_unrolled);
        matrix_multiply(H_unroll, M, W_unroll, x_unrolled, W, y.dtpr_);
    }
