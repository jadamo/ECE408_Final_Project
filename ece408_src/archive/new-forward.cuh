...
__global__ void forward_kernel(float *y, const float *x, const float *k, const int B, const int M, const int C, const int H, const int W, const int K)
{
  ...
  // y4d(0,0,0,0) = a
    ...
    */
    ...
    int h0 = threadIdx.x;
    int w0 = threadIdx.y;
    int h_base = (blockIdx.z / W_grid) * TILE_WIDTH*TILE_WIDTH;
    int w_base = (blockIdx.z % W_grid) * TILE_WIDTH*TILE_WIDTH;
    int h = h_base + h0;
    int w = w_base + w0;
    int c, p, q;
    // if (h < H_out && w < W_out) {
        ...
        for(c = 0; c < C; c++){
          ...
          for(p = 0; p < K; p++){
            for(q = 0; q < K; q++){
                if ((h+p) < X_tile_width && (w+q) < X_tile_width && p < K && q < K) {
                    value += X_shared[(h0+p)*X_tile_width + w0 + q] * W_shared[p*K + q];
                }
            }
          }
          __syncthreads();
        }
     if (n >= 0 && n <= B && m >= 0 && m < M && h >= 0 && h < H_out && w >= 0 && w < W_out) {
         y4d(n,m,h,w) = value;
    }
